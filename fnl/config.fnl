...
(map! [nvsict] "<C-\\>b" "<cmd>Telescope buffers<CR>" {:desc "open buffer"})

(map! [nvsict] "<C-\\>f" "<cmd>Telescope current_buffer_fuzzy_find<CR>"
      {:desc "search buffer"})
...
(map! [v] "#"
      (fn []
        (search-repeat-change.visualstar :backwards true))
      {:desc "visual star (backwards)"})

(map! [n] "#" "mz#`z" {:desc "star (backwards)"})
(map! [v] "*" (fn []
                (search-repeat-change.visualstar))
      {:desc "visual star"})
...
(map! [n] :<leader>ul (fn []
                        (vim.cmd :w)
                        (vim.cmd (.. "Fnlsource "
                                     (vim.fn.expand "~/.config/nvim/fnl/modules.fnl")))
...
(map! [n] :<leader>uv (fn []
                        ...
                        (vim.cmd (.. ":Fnlsource "
                                     (vim.fn.expand "~/.config/nvim/fnl/config.fnl")))
...
(map! [n] :<leader>nfN
      "<cmd> lua require'vt.telescope'.find_nvim({nvchad=true}) <CR>"
...
(map! [n] :<leader>ngp
      "<cmd> lua require'vt.telescope'.grep_nvim_plugins() <CR>"
      {:desc "grep neovim plugins"})

(map! [n] :K (fn []
               (vim.lsp.buf.hover)) {:desc "   lsp hover"}
      {:desc "   goto prev"})

(map! [n] :<C-j> :<C-w>j {:desc " window down"})
(map! [n] :<M-Up> ":m .-2<CR>==" {:desc "move up"})
(map! [n] :<leader>ngN
      "<cmd> lua require'vt.telescope'.grep_nvim({nvchad=true}) <CR>"
...
(map! [nvsit] "<C-\\>x" "<cmd>Telescope command_history<CR>"
      {:desc "telescope command history"})

(fn go-struct-to-python []
  (vim.cmd "'<,'>s/^\\t/    /ge | '<,'>s://:#:ge | '<,'>v:\\v^\\s*(#|//|$):normal ^crsEa:\027^")
  (vim.cmd "normal gv")
