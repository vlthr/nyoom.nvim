...
(fn path.Path [...] ((. plenary-path "Path::new") ...))
...
(fn path.strip-prefix [p prefix]
  ...
  (var prefix-literal (s.escape-pattern (s.strip-suffix prefix path.os-path-sep)))
  (let [(stripped _) (string.gsub p (.. "^" prefix-literal path.os-path-sep "+" "(.-)") "%1")]
    stripped))
...
(fn path.strip-suffix [p suffix]
  ...
  (if (= nil suffix)
    ...
    (do
      (var suffix-literal (s.escape-pattern (s.strip-prefix suffix path.os-path-sep)))
      (let [(stripped _) (string.gsub p (.. "(.-)" path.os-path-sep "+" suffix-literal) "%1")]
        stripped))))
...
(fn path.stdpath [kind ...]
  ...
  (var base (match kind
             ...
             :mason-packages (path.concat (vim.fn.stdpath :data) :mason :packages)
             :mason-packages-build (path.concat (vim.fn.stdpath :data) :mason :.packages)
